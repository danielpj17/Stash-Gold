...
/**
 * `accountBalancesTotal` is the sum of every computed account balance
 * (see `computeAccountBalances`) and stands in for liquid assets. It is passed
 * in rather than fetched here because the caller already derives it from the
 * cached transaction/transfer data.
 */
export async function getNetWorthSummary(
  month?: string,
  accountBalancesTotal = 0
  ...
  const liquidAssetsTotal = asFiniteNumber(accountBalancesTotal);
